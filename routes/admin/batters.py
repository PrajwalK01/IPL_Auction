"""routes/admin/batters.py — CRUD for Batsman players."""
...
batters_bp = Blueprint("admin_batters", __name__, url_prefix="/admin/batters")
...
def _save_photo(file):
    ...
    if ext not in ALLOWED_EXT:
        ...
        return None
    try:
        return upload_file_to_firebase(file, "players/batters")
...
def list_batters():
    try:
        ...
        flash(f"Error loading batters: {e}", "danger")
        players = []
    return render_template("admin/players.html", players=players,
                           player_type="Batters", collection=COLLECTION)


@batters_bp.route("/add", methods=["GET", "POST"])
@admin_required
def add_batter():
    db = get_db()
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"Batter {player_id} added! ✅", "success")
            return redirect(url_for("admin_batters.list_batters"))
        except Exception as e:
            flash(f"Error adding batter: {e}", "danger")

    cfg = get_app_config()
    return render_template("admin/batter_form.html", player=None, action="Add",
                           nationalities=sorted(cfg.get("nationalities", [])),
                           batting_styles=cfg.get("batting_styles", []),
                           player_types=cfg.get("player_types", []))


@batters_bp.route("/<string:pid>/edit", methods=["GET", "POST"])
@admin_required
def edit_batter(pid):
    db = get_db()
    try:
        ...
        if not player_doc.exists:
            flash("Batter not found.", "warning")
            return redirect(url_for("admin_batters.list_batters"))
        ...
        if request.method == "POST":
            ...
            flash("Batter updated successfully! ✅", "success")
            return redirect(url_for("admin_batters.list_batters"))
    except Exception as e:
    ...
    cfg = get_app_config()
    return render_template("admin/batter_form.html", player=player, action="Edit",
                           nationalities=sorted(cfg.get("nationalities", [])),
                           batting_styles=cfg.get("batting_styles", []),
                           player_types=cfg.get("player_types", []))


@batters_bp.route("/<string:pid>/delete", methods=["POST"])
@admin_required
def delete_batter(pid):
    try:
        db = get_db()
        ...
        flash("Batter removed successfully.", "info")
    except Exception as e:
        flash(f"Error: {e}", "danger")
    return redirect(url_for("admin_batters.list_batters"))


@batters_bp.route("/add-option", methods=["POST"])
@admin_required
def add_option():
    data = request.json
