"""routes/admin/bowlers.py — CRUD for Bowler players."""
...
bowlers_bp = Blueprint("admin_bowlers", __name__, url_prefix="/admin/bowlers")
...
def _save_photo(file):
    ...
    if ext not in ALLOWED_EXT:
        ...
        return None
    try:
        return upload_file_to_firebase(file, "players/bowlers")
...
def list_bowlers():
    try:
        ...
        flash(f"Error loading bowlers: {e}", "danger")
        players = []
    return render_template("admin/players.html", players=players,
                           player_type="Bowlers", collection=COLLECTION)


@bowlers_bp.route("/add", methods=["GET", "POST"])
@admin_required
def add_bowler():
    db = get_db()
    if request.method == "POST":
        ...
        try:
            db.collection(COLLECTION).document(player_id).set({
                ...
                "bowling_arm": sanitize_input(f.get("bowling_arm", "")),
            ...
            flash(f"Bowler {player_id} added! ✅", "success")
            return redirect(url_for("admin_bowlers.list_bowlers"))
        except Exception as e:
            flash(f"Error adding bowler: {e}", "danger")

    cfg = get_app_config()
    return render_template("admin/bowler_form.html", player=None, action="Add",
...
@bowlers_bp.route("/<string:pid>/edit", methods=["GET", "POST"])
@admin_required
def edit_bowler(pid):
    db = get_db()
    try:
        ...
        if not player_doc.exists:
            flash("Bowler not found.", "warning")
            return redirect(url_for("admin_bowlers.list_bowlers"))
    ...
    cfg = get_app_config()
    return render_template("admin/bowler_form.html", player=player, action="Edit",
                           nationalities=sorted(cfg.get("nationalities", [])),
                           bowling_styles=sorted(cfg.get("bowling_styles", [])),
                           batting_styles=cfg.get("batting_styles", []),
                           player_types=cfg.get("player_types", []))
...
@bowlers_bp.route("/<string:pid>/delete", methods=["POST"])
@admin_required
def delete_bowler(pid):
    try:
        db = get_db()
        ...
        flash("Bowler removed successfully.", "info")
    except Exception as e:
        flash(f"Error: {e}", "danger")
    return redirect(url_for("admin_bowlers.list_bowlers"))


@bowlers_bp.route("/add-option", methods=["POST"])
@admin_required
def add_option():
    data = request.json
