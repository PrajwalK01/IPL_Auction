"""routes/admin/mentors.py — CRUD for Mentors and Coaches (snake_case)."""
...
from db.helpers import sanitize_input
...
def list_mentors():
    try:
        ...
        mentors.sort(key=lambda x: x.get("name", ""))
    except Exception as e:
...
def add_mentor():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.collection("mentors").add({
                "name": sanitize_input(f["name"]),
                "role": sanitize_input(f["role"]),
                "team_id": tid,
                "salary": salary,
                "photo": photo,
                "is_active": True,
    ...
    teams = [d.to_dict() | {"id": d.id}
             for d in db.collection("teams")
             .where("is_deleted", "==", False).stream()]
    return render_template("admin/mentor_form.html", mentor=None,
                           action="Add", teams=teams)
...
def delete_mentor(mid):
    try:
        ...
        # Reverse the salary from team mgmt_spent
