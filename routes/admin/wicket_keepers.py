"""routes/admin/wicket_keepers.py — CRUD for Wicket Keeper players."""
...
wicket_keepers_bp = Blueprint("admin_wicket_keepers", __name__,
                              url_prefix="/admin/wicket-keepers")
...
def _save_photo(file):
    ...
    if ext not in ALLOWED_EXT:
        ...
        return None
    try:
        return upload_file_to_firebase(file, "players/wicket_keepers")
    except Exception as e:
        flash(f"Upload failed: {e}", "danger")
        return None


@wicket_keepers_bp.route("/")
@admin_required
def list_wicket_keepers():
    try:
        db = get_db()
        ...
        flash(f"Error loading wicket keepers: {e}", "danger")
        players = []
    return render_template("admin/players.html", players=players,
                           player_type="Wicket Keepers", collection=COLLECTION)


@wicket_keepers_bp.route("/add", methods=["GET", "POST"])
@admin_required
def add_wicket_keeper():
    db = get_db()
    if request.method == "POST":
        ...
        try:
            db.collection(COLLECTION).document(player_id).set({
                ...
                "stumpings": int(f.get("stumpings", 0)) if f.get("stumpings") else 0,
                "catches": int(f.get("catches", 0)) if f.get("catches") else 0,
            ...
            flash(f"Wicket Keeper {player_id} added! ✅", "success")
            return redirect(url_for("admin_wicket_keepers.list_wicket_keepers"))
        except Exception as e:
            flash(f"Error adding wicket keeper: {e}", "danger")

    cfg = get_app_config()
    return render_template("admin/wicket_keeper_form.html", player=None, action="Add",
                           nationalities=sorted(cfg.get("nationalities", [])),
                           batting_styles=cfg.get("batting_styles", []),
                           player_types=cfg.get("player_types", []))


@wicket_keepers_bp.route("/<string:pid>/edit", methods=["GET", "POST"])
@admin_required
def edit_wicket_keeper(pid):
    db = get_db()
    try:
        player_ref = db.collection(COLLECTION).document(pid)
        player_doc = player_ref.get()
        if not player_doc.exists:
            flash("Wicket Keeper not found.", "warning")
            return redirect(url_for("admin_wicket_keepers.list_wicket_keepers"))
        ...
        if request.method == "POST":
            ...
            flash("Wicket Keeper updated successfully! ✅", "success")
            return redirect(url_for("admin_wicket_keepers.list_wicket_keepers"))
    except Exception as e:
    ...
    cfg = get_app_config()
    return render_template("admin/wicket_keeper_form.html", player=player, action="Edit",
                           nationalities=sorted(cfg.get("nationalities", [])),
                           batting_styles=cfg.get("batting_styles", []),
                           player_types=cfg.get("player_types", []))


@wicket_keepers_bp.route("/<string:pid>/delete", methods=["POST"])
@admin_required
def delete_wicket_keeper(pid):
    try:
        db = get_db()
        ...
        flash("Wicket Keeper removed successfully.", "info")
    except Exception as e:
        flash(f"Error: {e}", "danger")
    return redirect(url_for("admin_wicket_keepers.list_wicket_keepers"))


@wicket_keepers_bp.route("/add-option", methods=["POST"])
@admin_required
def add_option():
    data = request.json
